...
- `mcp/session/discovery.ts`: pure functions over the MCP `Client` for tools/resources/prompts plus `complete()` (forwards `completion/complete` requests, projects `total`/`hasMore`). `SessionManager.complete` gates the call on the server's `completions` capability and throws `COMPLETIONS_NOT_SUPPORTED` if absent.
